...
def filter_since(items: List[Dict], since_dt: Optional[dt.datetime], keep_no_pub: bool = True) -> List[Dict]:
    if not since_dt: return items
    out: List[Dict] = []
    for it in items:
        pub = it.get("published")
        if not pub:
            if keep_no_pub: out.append(it)
            continue
        try:
            if dtparse.isoparse(pub) >= since_dt: out.append(it)
        except Exception:
            if keep_no_pub: out.append(it)
    return out
...
def collect_categories(categories: List[str], country: str, page_size: int,
    ...
    if not api_key: raise RuntimeError("NEWSAPI_KEY 필요")
    conn = connect_db()
    since_dt = None
    if since_hours is not None:
        since_dt = dt.datetime.now(tz=tz.UTC) - dt.timedelta(hours=since_hours)
        if debug: print(f"[Filter] since={since_dt.isoformat()} UTC")
    ...
    for cat in categories:
        ...
        if debug: print(f"[Filter] {cat}: {before} -> {len(filtered)}")
        filtered.sort(key=lambda x: x.get("published") or "", reverse=True)
        if limit_per_cat: filtered = filtered[:limit_per_cat]
        saved = skipped = 0
        ...
        if to_json: dump.extend(filtered)
    if to_json:
        with open(to_json, "w", encoding="utf-8") as f: json.dump(dump, f, ensure_ascii=False, indent=2)
    return results
...
def collect_categories_domains_mode(categories: List[str], page_size: int,
    ...
    for cat in categories:
        ...
        if not dom_csv:
            if debug: print(f"[Domains] {cat}: none")
        ...
        if debug: print(f"[Domains] {cat}: {before} -> {len(filtered)} (langs={','.join(languages)})")
        for it in filtered: it["category"] = cat
        filtered.sort(key=lambda x: x.get("published") or "", reverse=True)
        if limit_per_cat: filtered = filtered[:limit_per_cat]
        saved = skipped = 0
